...
export const createForm = create((set) => ({
    show: false,
    setShow: (show) => set({show}),
    showCreateFormDialog: () => set({show: true}),
    hideFormDialog: () => set({show: false}),
   }));


function CreateForm({msg, accept, reject, acceptButtonText= "accept", rejectButtonText="reject" , inputs=[]}) {
    const {show, hideFormDialog} = createForm()
