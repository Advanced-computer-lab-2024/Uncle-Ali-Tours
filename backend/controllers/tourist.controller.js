...
export const addActWishlist = async (req, res) => {
    const { userName, _id } = req.body;
    try {
        const tourist = await Tourist.findOne({ userName });
        if (!tourist) {
        ...
        // Check if product is already in wishlist
        if (tourist.addActWishlist.includes(_id)) {
            return res.status(404).json({ success: false, message: "Already bookmarked" });
        }

        // Add product to wishlist
        tourist.bookmark.push(_id);
        await tourist.save(); // Save changes to the database

        // Send the updated wishlist back in the response
        return res.status(200).json({
            success: true,
            data: tourist.bookmark,  // Return the updated wishlist
            message: 'Added to wishlist successfully'
        });
    } catch (error) {
        console.error("Error Adding to Wishlist:", error);
...
export const removeActWishlist = async(req,res) => {
    const {userName , _id} = req.body;
    try{
        const tourist = await Tourist.findOne({ userName });
        if (!tourist) {
        ...
        console.log(_id)
        if(!tourist.bookmark.includes(_id)){
            return res.status(404).json({ success: false, message: "not in wishlist" });
        }
        ...
        tourist.bookmark = tourist.bookmark.filter(item => item !==_id);
        await tourist.save();
        return res.status(200).json({ success: true, data: tourist.myPreferences, message: 'removed successfully' });
        
    }catch (error) {
        console.error("Error :", error);
        res.status(500).json({ success: false, message: "Server error during Removing" });
    }
}
...
export const getActProducts = async (req, res) => {
    const { userName } = req.params; // Get userName from request parameters

    try {
        // Find tourist by username and populate productsWishlist
        const tourist = await Tourist.findOne({ userName }).populate('bookmark');
        ...
        // Find the products in the wishlist
        const bookActivity = await Activity.find({ _id: { $in: tourist.bookmark } });

        // Return success response with wishlisted products
        return res.status(200).json({ success: true, data: bookActivity });
    } catch (error) {
        console.error("Error Fetching Wishlisted Products:", error);
        res.status(500).json({ success: false, message: "Server error during fetching wishlisted products" });
    }
